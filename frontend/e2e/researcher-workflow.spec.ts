...
test('first boot, project CRUD, focused scan and phase ledger', async ({ page }, testInfo) => {
  ...
  await expect(scanDialog).toBeInViewport()
  await expect(scanDialog.getByText('Scan profile', { exact: true })).toBeInViewport()
  await expect(scanDialog.getByRole('button', { name: /Start Scan/ })).toBeInViewport()
  const dialogBox = await scanDialog.boundingBox()
  const viewport = page.viewportSize()
  expect(dialogBox, 'scan dialog must have a measurable viewport box').not.toBeNull()
  expect(viewport, 'scan workflow must run with a fixed viewport').not.toBeNull()
  expect(dialogBox!.y).toBeGreaterThanOrEqual(0)
  expect(dialogBox!.y + dialogBox!.height).toBeLessThanOrEqual(viewport!.height)
  await scanDialog.locator('[data-modal-scroll-region]').evaluate(element => { element.scrollTop = element.scrollHeight })
  await expect(scanDialog.getByRole('button', { name: /Start Scan/ })).toBeInViewport()
